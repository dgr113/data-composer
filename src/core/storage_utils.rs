...
pub fn read_json( file_path: &str ) -> serde_json::Value {
    fs::read_to_string( file_path )
        .and_then( |content| Ok( serde_json::from_str( &content ).expect( "Error convert JSON string into Value!" ) ) )
        .unwrap_or( Value::Null )
}


/** Prepare one element to doc */
pub fn prepare_to_doc(d: Option<&serde_json::Value>, id_field: Option<&str>) -> Option<Document> {
    d.and_then( |t| {
        if t.is_object() {
            Some( t.to_owned().try_into().unwrap() )
        }
        else if t.is_array() {
            Some( t[0].to_owned().try_into().unwrap() )
        }
        else {
            None
        }
...
pub fn mongo_get_data(coll: &Collection, filter: Document) -> Vec<Document> {
...
pub fn mongo_convert_results( results: Vec<Document> ) -> Vec<serde_json::Value> {
...
pub fn check_coll_exists( coll: &Collection ) -> bool {
    match coll.find_one(Some( Document::new() ), None) {
        Ok( t ) => t.is_some(),
        Err( _ ) => false
    }
}
